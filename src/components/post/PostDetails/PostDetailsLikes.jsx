...
export default function PostDetailsLikes({ post }) {
  const { auth } = useAuth();
  const { api } = useAxios();
  ...
  useEffect(() => {
    setLiked(post?.likes?.some((item) => item._id === auth.user._id));
    setLikedusers(post?.likes || []);
  }, [post, auth.user._id]);
  ...
  return (
    <div className="p-3 border-b">
        ...
        <div className="flex space-x-4">
          <button onClick={handleLike} className="cursor-pointer">
            <svg
              ...
              className={`h-6 w-6 ${
                liked ? "stroke-red-600" : "stroke-zinc-600"
              }`}
              fill={`${liked ? "red" : "none"}`}
              viewBox="0 0 24 24"
              stroke="currentColor"
      ...
      {/* <!-- Likes --> */}
      <div className="mb-1">
        <p className="text-sm font-semibold">{likedUsers.length} likes</p>
      </div>
